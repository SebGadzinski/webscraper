...
provinceInput = tkinter.ttk.Combobox(window,
                                     values=["NS", "QC", "ON", "MB", "SK", "AB", "BC", "NT", "NB", "NB", "YT", "PE",
                                             "NU"])
provinceInput.grid(row=2, column=1, sticky=tkinter.NSEW)
provinceInput.current(2)
btn = tkinter.Button(window, bg='#ADD8E6', text="Send To CSV")

def clicked():
    loadInfoForCSV(buisnessInput.get(), cityInput.get(), provinceInput.get())

btn.config(command=clicked)
btn.grid(row=3, column=0, columnspan=2, sticky=tkinter.W + tkinter.E)
